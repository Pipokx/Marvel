...
const ComicsPage = ({ token }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get(`${baseURL}/comics?page=${currentPage}&title=${searchQuery}`);
        ...
        <>
          <section className="category">
            {data.results.map(item => {
              const isFavorited = favorites.some(f => String(f.entityId) === String(item._id || item.id) && f.type === 'comic');
